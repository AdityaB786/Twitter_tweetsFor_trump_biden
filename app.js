...
const liveUrl = process.env.LIVE_URL || 'http://localhost:3000'; // Use LIVE_URL from environment or default to localhost
...
app.get('/run-script', async (req, res) => {
    ...
    try {
        ...
        const uri = process.env.MONGO_URI || 'mongodb://localhost:27017'; // Use Mongo URI from environment or fallback to local
...
app.listen(port, () => {
    console.log(`Server running at ${liveUrl}:${port}`);
});
